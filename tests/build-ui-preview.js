...
const templates = {};
for (const name of ['index', 'quiz', 'result']) templates[name] = parse(read(`pages/${name}/${name}.wxml`));
for (const name of ['type-card', 'quadrant-map']) templates[name] = parse(read(`components/${name}/${name}.wxml`));
...
function render(nodes, data) {
  ...
  for (const node of nodes) {
    ...
    const tag = ({ view: 'div', text: 'span', 'scroll-view': 'div', image: 'img' })[node.tag] || node.tag;
...
function screen(page, data) {
  const style = commonCss + css(`pages/${page}/${page}.wxss`);
...
const base = { copy, assets, sharingCopy, pairCopy };
